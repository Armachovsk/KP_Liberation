...
params [
    ...
    ["_dir", 0, [0]],
    ["_active", false, [false]],
    ["_ied_list", SPEC_ied_list, [[]]]
];

if (KPLIB_param_debug) then {
    [format ["%1", _this]] call SPEC_fnc_core_debugLog;
};
...
private _ied = createMine [selectRandom SPEC_preset_iedsACE, [_pos select 0, _pos select 1, SPEC_ied_offset], [], 2];
