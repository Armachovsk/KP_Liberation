...
[{
    ...
    if (alive _driver_drone && !isNull _driver_drone) then {
        private _array = _driver_drone nearEntities [SPEC_preset_playerF, 200];
        if (_array isEqualTo []) then {
            ...
            if (_trigger isEqualTo []) then {
                _trigger pushBack ([_driver_drone] call SPEC_fnc_ied_droneActive);
            };
        ...
        _this call SPEC_fnc_ied_droneLoop;
    } else {
        deleteVehicle (_trigger deleteAt 0);
        _group setVariable ["btc_ied_drone", false];

        if (KPLIB_param_debug) then {
            [format ["_driver_drone = %1 POS %2 END LOOP", _driver_drone, getPos _driver_drone]] call SPEC_fnc_core_debugLog;
        };
    };
